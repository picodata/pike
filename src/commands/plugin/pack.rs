...
pub fn cmd(
        ...
        if let Some(members) = workspace.get("members").and_then(|m| m.as_array()) {
            for member in members {
                ...
                if member_path.join("manifest.yaml.template").exists() {
                    ...
                    create_plugin_archive(&build_root, &member_path, no_build)?;
                    packaged_any = true;
                } else {
                    debug!(
    ...
    create_plugin_archive(&build_root, &root_dir, no_build)
}

fn create_plugin_archive(build_dir: &Path, plugin_dir: &Path, no_build: bool) -> Result<()> {
    ...
    // When --no-build is on, ensure build tree is valid before packing
    if no_build {
