...
pub fn cmd(pack_debug: bool, target_dir: &PathBuf, plugin_path: &PathBuf) -> Result<()> {
    ...
    cargo_build(build_type, target_dir, plugin_path).with_context(|| {
        format!(
            "building {} version of plugin",
            match build_type {
    ...
    let build_root = {
        ...
        match build_type {
            BuildType::Debug => effective_target_dir.join("debug"),
            BuildType::Release => effective_target_dir.join("release"),
        }
    };
